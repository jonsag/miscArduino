...
boolean voidLoop = false; // main program started
byte i;
...
void setup() {
  ...
  Serial.println("Counting sensors...");
  numSensors = discoverOneWireDevices(); // store the number of sensors
  int tempValue[numSensors]; // array with "numSensors" storage places for the temperature of each sensor
  if(numSensors > 0) {
...
void loop() {
      ...
      if (client.available()) {
        ...
        if (c == '\n') { //if HTTP request has ended
          ...
          if(readString.indexOf("pulse") > 0) { //checks for click to simulating a pulse
            pulsesThisInterval++; // +1 on  pulse counters
            pulses++;
          ...
          if(readString.indexOf("readTemps") > 0) { //will read 1-wire temps
            ...
            for(i = 0; i < numSensors; i++) {
              int temp = sensors.getTempCByIndex(i);
              tempValue[i] = temp;
              Serial.print("Temp");
              Serial.print(i);
              Serial.print(": ");
  ...
  if (timeStatus() == timeNotSet || elapsedSyncMillis > 86400000) { // sync clock
    setTime();
  }
  ///////////////////// blink LEDs
...
void setTime() {
  ...
  Serial.println("---Sent NTP Packet and waiting for reply...");
  delay(1000); // wait to see if a reply is available
  if ( Udp.parsePacket() ) { // We've received a packet, read the data from it
    Serial.println("---Recieved packet, reading it's data...");
  ...
  Serial.println("Finished syncing clock");
