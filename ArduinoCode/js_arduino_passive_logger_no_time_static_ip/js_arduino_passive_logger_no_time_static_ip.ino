...
Define these variables down in the code:
 ...
 if (!tempsRead || tempAge > 60000 -----how often should we read temps, milliseconds
 if (ii < 200) ----- how many loops should we calculate an average from
...
int iiii = 0; // counter for how often to read currents
...
void loop() {
        ...
        if (c == '\n') { //if HTTP request has ended
          ...
          if(readString.indexOf('?') >=0) { //don't send new page
            ...
            for(i=1; i <= 3; i++) {
              ...
              client.print(i);
              client.print(": ");
              client.print(displayedCurrent[i]);
          ...
          if(readString.indexOf("pollReset") > 0) { //checks for poll reset
            ...
            pulses =0;
            for(i=1; i <= 3; i++) {
              polledCurrent[i] = ackPolledCurrent[i] / iii; // set the displayed current value
              ackPolledCurrent[i] = 0;
            }
            iii = 0;
          }
  ...
  ///////////////////// read analog inputs
  if (!readTemps) { // if we are not reading temps
    if (iiii >= 0) { // only read temps this often
      if (ii < 200) {
        current[1] = abs((analogRead(current1) - 511) * 0.33); // read currents
        current[2] = abs((analogRead(current2) - 511) * 0.33);
        current[3] = abs((analogRead(current3) - 511) * 0.33);
        /*
        ...
        for(i=1; i <= 3; i++) {
          ...
          Serial.print(i);
           ...
           if (current[i] > 100) { // check for abnormal value
           //if((current[i] - lastCurrent[i]) > 30) {
           current[i] = lastCurrent[i];
           }
           else {
           ...
           Serial.print(" A     ");
           */
          ackDisplayedCurrent[i] = ackDisplayedCurrent[i] + current[i];
        }
        /*        
         Serial.println();
         */
        ...
        for(i=1; i <= 3; i++) {
          displayedCurrent[i] = ackDisplayedCurrent[i] / 200; // set the displayed current value

          Serial.print(i);
          Serial.print(": ");
          Serial.print(displayedCurrent[i]);
          Serial.print(" A     ");

          ackDisplayedCurrent[i] = 0;
        }
        Serial.println();
        ...
        for(i=1; i <= 3; i++) {
          ackPolledCurrent[i] = ackPolledCurrent[i] + displayedCurrent[i]; // calculate an average for interval
        }

        iii++;
      }

      iiii = 0; // reset counter
    }
    else {
      iiii++;
    }
