...
Define these variables down in the code:
 ------------------------------------
 byte mac[] = the number on the sticker on your ethernet shield
 IPAddress ip( the IP number you want yor arduino board to use
 if ( elapsedResetMillis > how long is the intervals between reset of puls counting
 if (!tempsRead || elapsedTempsMillis > how often should we read temps
...
int ii = 0; // counter for current average
int iii = 0; // counter för current interval average
...
void loop() {
      ...
      if (client.available()) {
        ...
        if (c == '\n') { //if HTTP request has ended
          ...
          if(readString.indexOf('?') >=0) { //don't send new page
            ...
            client.print(elapsedTempsMillis/1000);
            ...
            client.print(pulses);
            client.println("<br>");
            client.print("This interval is ");
            client.print(elapsedResetMillis/1000);
            client.println(" seconds old");
            client.println("<br>");
            client.println("<br>");
          ...
          if(readString.indexOf("pollReset") > 0) { //checks for poll reset
            ...
            pulses =0;
            for(i=1; i <= 3; i++) {
              currentIntervalValue[i] = currentIntervalAverageValue[i] / iii; // set the displayed current value
              currentIntervalAverageValue[i] = 0;
            }
            iii = 0;
            polled = true;
          }
  ...
  if (ii < 200) {
    ...
    // phase 1
    readCurrent[1] = abs((analogRead(current1) - 511) * 0.33);
    if (readCurrent[1] > 50 || readCurrent[1] < 0) {
      readCurrent[1] = lastCurrentValue[1];
    }
    else {
      lastCurrentValue[1] = readCurrent[1];
    }

    // phase 2
    readCurrent[2] = abs((analogRead(current2) - 511) * 0.33);
    if (readCurrent[2] > 50 || readCurrent[2] < 0) {
      readCurrent[2] = lastCurrentValue[2];
    }
    else {
      lastCurrentValue[2] = readCurrent[2];
    }

    // phase 3
    readCurrent[3] = abs((analogRead(current1) - 511) * 0.33);
    if (readCurrent[3] > 50 || readCurrent[3] < 0) {
      readCurrent[3] = lastCurrentValue[3];
    }
    else {
      lastCurrentValue[3] = readCurrent[3];
    }
    
    for(i=1; i <= 3; i++) {
      currentAverageValue[i] = currentAverageValue[i] + readCurrent[i];
    }
    ...
    for(i=1; i <= 3; i++) {
      currentValue[i] = currentAverageValue[i] / 200; // set the displayed current value
      currentAverageValue[i] = 0;
    }
    ii = 0;
    for(i=1; i <= 3; i++) {
      currentIntervalAverageValue[i] = currentIntervalAverageValue[i] + currentValue[i]; // calculate an average for interval
    }
    iii++;
  }
  ...
  ///////////////////// check if we should read temps
  if (!tempsRead || elapsedTempsMillis > 60000) { // read temperatures if we haven't before, or if it's time to do so
    getTemperatures();
  }
