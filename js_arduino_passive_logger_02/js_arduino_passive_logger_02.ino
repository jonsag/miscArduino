...
float currentValue[] = { // sensorValue holds the analog inputs
  1, 2, 3};
...
boolean mainLoop = false; // is main loop started
...
void loop() {
  ...
  currentValue[1] = analogRead(current1) * 100.0 / 1024.0; // convert the analog inputs to a sensible value
  currentValue[2] = analogRead(current2) * 100.0 / 1024.0;
  currentValue[3] = analogRead(current3) * 100.0 / 1024.0;
  ...
  if (pulseButtonState != lastPulseButtonState) { // light red LED if button is pressed
    if (pulseButtonState == HIGH) { // if the current state is HIGH then the button went from off to on
      pulsesThisInterval++; // +1 on all pulse counters
      pulses++;
      Serial.print("Pulse from button, pulses thsi interval is now "); 
      Serial.println(pulsesThisInterval);
      blinkRed = true;
    }
  }
